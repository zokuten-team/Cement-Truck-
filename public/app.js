...
function bind() {
  ...
  document.querySelectorAll("[data-action='audit']").forEach(b=>b.onclick=showAuditModal);
...
async function showAuditModal() {
  document.querySelector("#modal-root").innerHTML = `<div class="modal-backdrop"><div class="modal"><button class="modal-close" data-close>×</button><span class="eyebrow">Audit Log</span><h2>Recent changes</h2><p>Loading history...</p></div></div>`;
  try {
    const res = await fetch("/api/audit");
    const logs = await res.json();
    const rows = logs.length ? logs.map(l => `<div class="payment-row"><div><strong>${escapeHtml(l.actor)}</strong><span>${escapeHtml(l.action)} · ${shortDate.format(dateObj(l.timestamp.split('T')[0]))} ${l.timestamp.slice(11,16)}</span></div><b style="font-size:0.85rem;color:var(--text);font-weight:normal;max-width:200px;text-align:right">${escapeHtml(l.details)}</b></div>`).join("") : `<div class="empty-history">No changes recorded yet.</div>`;
    modal(`<button class="modal-close" data-close>×</button><span class="eyebrow">Audit Log</span><h2>Recent changes</h2><p>Tracked actions across all users.</p><div class="payment-history" style="max-height:60vh;overflow-y:auto;margin-top:1rem">${rows}</div>`);
    document.querySelectorAll("[data-close]").forEach(b=>b.onclick=closeModal);
  } catch(e) {
    toast("Failed to load audit log");
    closeModal();
  }
}
